...
- **SY DLP integration** (`shared/yeoman_dlp.py`) — crew output optionally routed through SecureYeoman's DLP pipeline before returning to caller. `YEOMAN_DLP_ENABLED`, `_URL`, `_API_KEY`, `_BLOCK_ON_FINDING`, `_TIMEOUT` env vars. Integrated into `_run_crew_async` result pipeline
